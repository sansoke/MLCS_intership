...
def create_sequences(data, seq_length):
    ...
    for i in range(len(data) - seq_length):
        sequences.append(data[i:i + seq_length])  # Full feature set
        targets.append(data[i + seq_length, 0])   # Predicting 'Close' price
...
# torch.save(model.state_dict(), 'many_to_one.pth')

model.load_state_dict(torch.load('many_to_one.pth'))
# """
...
predictions = scaler_close.inverse_transform(np.array(predictions).reshape(-1, 1))
actuals = scaler_close.inverse_transform(np.array(actuals).reshape(-1, 1))
